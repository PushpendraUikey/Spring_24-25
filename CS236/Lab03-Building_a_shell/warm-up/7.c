...
int main() {
    ...
    sigaction(SIGCHLD, &sa, NULL);      // handles only the sigchld signal
    // "Whenever a SIGCHLD arrives, call handle_sigchld()."
    // "Don't notify me if a child stops (SA_NOCLDSTOP)."
    // "Restart interrupted system calls (SA_RESTART)." syscalls can return prematurely if signal intrrupted
