...
"""
Do not import any other package unless allowed by TAs in charge of the lab.
...
def findposition(val, arr: np.ndarray) -> Tuple[int, int]:
    ...
    for i in range(n):
        for j in range(n):
            if arr[i][j] == val:
                return i, j
    return -1, -1   # this won't achieved
...
def bfs(initial: np.ndarray, goal: np.ndarray) -> Tuple[List[str], int]:
    ...
    while openlist:
        ...
        for indx, move in enumerate(neighbors):
            ...
            # No need of heuristic calculation in plain bfs.
...
def astar_dt(initial: np.ndarray, goal: np.ndarray) -> Tuple[List[str], int, int]:
    ...
    closedlist = []
    first = Node(initial)
    first.h = heuristic_dt(initial, goal)
    first.f = first.h
    heapq.heappush(openlist, first)
    nodesexplored = 0
    ...
    while openlist:
        ...
        if(ifequal(current.state, goal)):
            ...
            path = []
            while current.parent:
                path.insert(0,current.m)  # I don't think it is efficient.
                current = current.parent
            return path, nodesexplored, cost
...
def astar_md(initial: np.ndarray, goal: np.ndarray) -> Tuple[List[str], int, int]:
    ...
    while openlist:
        ...
        for indx, move in enumerate(neighbors):
            temp = current.state.copy()
            x, y = zeroposition(temp)
            nx = x + move[0]    # make new variables
            ny = y + move[1]
