    ...
    while open:
        ...
        if np.array_equal(top.board, goal):
            ...
            while curr.dir != '':
                seq.append(curr.dir)
                curr = curr.parent
            seq = seq[::-1]     # reverses the list...
            return (seq, nodes)
        
        row, col = np.where(top.board == 0) # check at what position zero is located.
        row = row[0]
        col = col[0]    # in our case only the one position has zero, thus zeroth index is chosen 
                # otherwise one has to decide which position's zeroth index has to be chosen.
